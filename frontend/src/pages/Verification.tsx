...
const schema = z.object({
    code: z.string().length(6, "Code must be exactly 6 digits").regex(/^\d+$/, "Only numbers allowed"),
});
...
const Verification: React.FC = () => {
    ...
    const onSubmit = async (data: any) => {
        try {
            setLoading(true);
            const response = await userService.verifyUser({ email: state.email, code: data.code });

            if (response.status === 200) {
                notify(response?.data?.message ?? "Verify success. You can login now", "success");
                navigate("/login");
            }
        } catch (error: any) {
            console.error(error);
